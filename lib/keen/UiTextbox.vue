      ...
      <label class="ui-textbox-label">
        <div :class="['ui-input-content', inputContentClasses]" :clearable="clearable">
          <input v-if="!multiLine" ref="input" v-autofocus="autofocus" class="ui-textbox-input"
            ...
            :min="minValue" :name="name" :number="type === 'number' ? true : null"
            :placeholder="hasFloatingLabel ? null : placeholder" :readonly="readonly" :required="required"
            :step="stepValue" :style="isActive ? { borderBottomColor: $themeTokens.primaryDark } : {}"
            :tabindex="tabindex" :type="type" :value="value" @blur="onBlur" @change="onChange" @focus="onFocus"
            @input="updateValue($event.target.value)" @keydown.enter="onKeydownEnter" @keydown="onKeydown">
